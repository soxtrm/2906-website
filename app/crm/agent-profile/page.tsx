...
async function buildInvoicePdf(data: {
  ...
  // Kev, 2026-09-17: "wenn man oben die Bankdaten eingibt sollten die doch
  // auf der rechnung unten links sein" — payment details, bottom-left,
  // whenever the Agent's profile actually has them.
  if (data.agent.bank_iban || data.agent.bank_account_name) {
    ...
    doc.setTextColor(...INK)
    let py = payY + 14
    if (data.agent.bank_account_name) { doc.text(data.agent.bank_account_name, margin, py); py += 12 }
    if (data.agent.bank_iban) { doc.text(`IBAN: ${data.agent.bank_iban}`, margin, py); py += 12 }
  }
