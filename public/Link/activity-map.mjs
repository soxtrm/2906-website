...
export function createActivityMap(map,{send,reduced}){
 let active=false,origin=null,pois=[],selected=null,lastScan=0,signature='';
 function install(){
  ...
  map.addLayer({id:'nexus-activity-halo',type:'circle',source:'nexus-activities',paint:{'circle-radius':['case',['get','origin'],20,['get','selected'],16,10],'circle-color':['case',['get','origin'],'#7acaff',['get','selected'],'#ffe2a1','#9abff2'],'circle-opacity':.18,'circle-pitch-alignment':'map'}});
  map.addLayer({id:'nexus-activity-points',type:'circle',source:'nexus-activities',paint:{'circle-radius':['case',['get','origin'],7,5],'circle-color':['case',['get','origin'],'#70c8ff',['get','selected'],'#ffe2a1','#8dbefa'],'circle-stroke-width':2,'circle-stroke-color':'#122e4b'}});
  map.addLayer({id:'nexus-activity-labels',type:'symbol',source:'nexus-activities',layout:{'text-field':['get','name'],'text-font':['DIN Offc Pro Medium'],'text-size':11,'text-anchor':'bottom','text-offset':[0,-1.5],'text-padding':10},paint:{'text-color':'#e8f6ff','text-halo-color':'#10233c','text-halo-width':2}});
 }
 function paint(){
  if(!map.getSource('nexus-activities'))return;
  map.getSource('nexus-activities').setData(active?{type:'FeatureCollection',features:[feature(origin,{id:'origin',origin:true,selected:false,name:'Your place'}),...pois.map(p=>feature(p.coordinates,{id:p.id,name:p.name,origin:false,selected:p.id===selected}))]}:empty());
 ...
 map.on('idle',scan);
 setInterval(()=>{if(!active||document.hidden||reduced.matches||!map.getLayer('nexus-activity-line'))return;const phase=Math.floor(performance.now()/140)%4;map.setPaintProperty('nexus-activity-line','line-dasharray',[[0,2,1,0],[.3,2, .7,0],[.6,2,.4,0],[1,2]][phase]);},140);
 ...
 },click(event){if(!active)return false;const hit=map.queryRenderedFeatures(event.point,{layers:['nexus-activity-points','nexus-activity-labels','nexus-activity-halo']}).find(f=>f.properties.id!=='origin');if(hit){focus(hit.properties.id);send('activity-select',{id:hit.properties.id});}return true;}};
}
