...
export function installPropertyReel(host,{load,photoUrl,onSelect}){
 ...
 function render(properties){
  ...
  if(!compact)timer=setInterval(()=>{if(!host.isConnected){clearInterval(timer);clearTimeout(hitTimer);return;}if(document.hidden||reduced.matches||paused||hover||focused)return;if(performance.now()>=nextAt){change();nextAt=performance.now()+HERO_INTERVAL;}},100);
 }
