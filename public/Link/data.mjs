...
export function restoreProfile(value){
 ...
 p.anchors=Array.isArray(value.anchors)?value.anchors.slice(0,10).filter(a=>a&&['work','school','hobby','everyday','gym','sport'].includes(a.type)).map(a=>({type:a.type,person:String(a.person||'You').slice(0,40),label:String(a.label||'').slice(0,80),location:String(a.location||'').slice(0,120),address:String(a.address||'').slice(0,180),placeId:String(a.placeId||'').slice(0,180),days:Number.isInteger(a.days)&&a.days>=1&&a.days<=7?a.days:null,time:/^\d\d:\d\d$/.test(a.time)?a.time:'',coordinates:Array.isArray(a.coordinates)&&a.coordinates.length===2&&a.coordinates.every(Number.isFinite)?a.coordinates:null})):[];
